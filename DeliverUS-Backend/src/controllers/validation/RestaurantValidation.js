...
import { Restaurant, Sequelize } from '#root/src/models/models.js'
...
// Create a where clause object to filter the Restaurant model. The discount code and the ownerId must match.
  const whereClause = {
    ...
    discountCode // The discountCode field of the Restaurant model must match the discountCode parameter.
  }
...
// Create a where clause object to filter the Restaurant model. The discount code and the ownerId must match.
  const whereClause = {
    userId: ownerId, // The userId field of the Restaurant model must match the ownerId parameter.
    discountCode, // The discountCode field of the Restaurant model must match the discountCode parameter.
    // If a restaurantId is provided, it means we are updating arestaurant, so we need to exclude the current restaurant.
  ...
  // If no matching restaurants are found, return true to indicate thatthe discount code is not repeated.
  return true
}
...
const update = [
  ...
  check('discountCode').optional().isString().isLength({ min: 1, max: 10 }).trim(),
  check('discount').optional().isFloat({ min: 1, max: 99 }).toFloat(),
  check('discountCode').custom((value, { req }) => {
